...
class AutoencoderRegression(nn.Module):
    def __init__(self,autoencoder, in_channels=1, is_unpooling=True):
        ...
        self.down5 = autoencoder.down5
        self.fc = nn.ModuleList()
        num_ftrs = 8*8*512
        self.fc.append(nn.Linear(num_ftrs, 4096))
        self.fc.append(nn.ReLU(inplace = True))
        self.fc.append(nn.Dropout(p = 0.5))
        self.fc.append(nn.Linear(4096, 1))
    def forward(self, inputs):
        down1, indices_1, unpool_shape1 = self.down1(inputs)
        down2, indices_2, unpool_shape2 = self.down2(down1)
        down3, indices_3, unpool_shape3 = self.down3(down2)
        down4, indices_4, unpool_shape4 = self.down4(down3)
        f, indices_5, unpool_shape5 = self.down5(down4)
